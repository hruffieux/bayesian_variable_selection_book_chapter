...
between the SNPs and the expression levels from each datasets. The two steps: 
...
available in this repository - alternatively, one can directly use the 
...
stopifnot(all.equal(n, n_check)) # same number of samples for SNPs and traits within each eQTL problems
rm(n_check)
...
schedule on the inverse temperature, initial temperature of $2$ and $10$ 
...
We finally display a Manhattan-type plot which indicates the position of the
hotspots, as estimated in the five different eQTL analyses. Recall that we are 
using simulated data so these results do not reflect the real data analysis of 
