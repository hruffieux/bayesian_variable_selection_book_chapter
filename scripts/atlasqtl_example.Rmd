...
quantitative trait locus (eQTL) analyses using the Bayesian mapping approach 
ATLASQTL [ADD REF]. It is an online material for the [ADD REF] book chapter and 
uses data adapted from the monocyte eQTL analysis illustration discussed in 
Section [ADD REF]. 
...
using efficient variational inference algorithms augmented with simulated 
...
with proteomic, lipodomic or metabolic expression. 

### Data

The data used in this example consist of synthetic genotyping data which emulate 
the genotypes (single nucleotide polymorphisms - SNPs) used in Section [ADD REF] 
for the *LYZ* gene region (the latter require data access agreement and hence 
cannot be shared directly). The expression data consist of resting 
(unstimulated) monocytes, monocyte levels obtained from three stimulated 
conditions (IFNg, LPS2h, LPS24h) and B-cells; see Section [ADD REF] for a full 
description. Each of these transcriptomic datasets gives rise to its own eQTL 
problem. 

As we are using synthetic SNP data, we need to simulate genetic association 
between the SNPs and each of these expression datasets. The two steps: 
...
are done using our in-house R package `echoseq` available at [ADD REF]. While 1)
requires access to the real genotyping data, 2) can be run using the R script
`prepare_data.R` available in this repository - alternatively, one can directly 
use the `prepared_data.RData` R object file which contains the datasets 
ready-to-used for this tutorial. 
...
{r} [here] (https://github.com/hruffieux/atlasqtl#installation). Note that the 
...
change according to your own setup):
...
standard laptop, but please change this number according to your setup. 
Setting `n_cpus <- 1` corresponds to a serial execution.
...
stopifnot(all.equal(n, n_check)) # same number of samples for SNPs and traits in within eQTL problems
rm(n_check)
...
(with the "null-case" permutation runs using the same prior specifications). 
...
circumvent ad-hoc specification of high-level variances (a case where inference 
is prone to strong bias). Our specification also has desirable multiplicity 
adjustment properties for dealing with the large-response case, see [ADD REF].
...
overall run time. We are using the default annealing setting, namely with a 
geometric schedule on the inverse temperature, initial temperature of $2$ and 
$10$ different temperatures. This specification as well as alternative options
...
Note that parallel execution relies on the R package `parallel` which has 
...
The object return by `atlasqtl` contains a range of useful posterior quantities 
to assess, e.g., 
* pairwise associations between each pair of SNP and trait: using the 
variational posterior probabilities (PPIs) stored in `gam_vb` ($p \times q$ matrix) and 
the variational posterior means of the regression estimates stored in `beta_vb` 
($p \times q$ matrix);
* the hotspot propensities: using the variational posterior mean of $\theta_s$ 
stored in `theta_vb (vector of length $p$).

It also contains diagnostic values on the final status of convergence, number of
...
We finally display a Manhattan plot which shows the position of hotspots for the 
five different eQTL analyses. Recall that we are using simulated data so these 
do not reflect the real data analysis of Section [ADD REF] of the book
chapter, although the code is similar to that used for producing Figure [ADD REF].

Also note that the presented there implemented permutation analyses to compute a
calibrated Bayesian FDR threshold on the PPIs; we skip this for computational 
economy and use an arbitrary threshold instead. The procedure to derive these
permutation-based FDR threshold is described in details in [ADD REF VB and ATLAS].
