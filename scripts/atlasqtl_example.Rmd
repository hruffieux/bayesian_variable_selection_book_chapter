...
ATLASQTL whose R package is available 
...
algorithms" [@ruffieux2021variable] published in [@tadesse2021handbook], and 
uses data adapted from the monocyte eQTL analysis illustration discussed in 
Section [@ruffieux2020global]. 
...
the genotypes (single nucleotide polymorphisms - SNPs) used in Section 1.3.2 of 
the book chapter for the *LYZ* gene region (the latter require data access 
agreement and hence cannot be publicly shared). The expression data consist of 
resting (unstimulated) monocytes, monocyte levels obtained from three stimulated 
conditions (IFNg, LPS2h, LPS24h) and B-cells; see Section 1.3.2 for a full 
...
are done using our in-house R package `echoseq` available [here](https://github.com/hruffieux/echoseq). While 1)
...
do not reflect the real data analysis of Section 1.3.2 of the book chapter, 
although the code is similar to that used for producing Figure 1.4.
...
permutation-based FDR threshold is described in details in 
